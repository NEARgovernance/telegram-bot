...
["VOTING_CONTRACT", "TELEGRAM_BOT_TOKEN", "MONGO_URI", "WEBHOOK_URL"].forEach(
  (key) => {
    if (!process.env[key]) {
  ...
  try {
    ...
    eventClient.on("close", () => {
      ...
      if (reconnectAttempts < maxReconnectAttempts) {
        ...
        console.log(
          "⚠️ Event stream disabled, but webhook functionality continues"
        );
        isConnecting = false;
      }
    });
...
function extractProposalId(event) {
  return (
    event.event_data?.[0]?.proposal_id ||
    ...
    (Array.isArray(event.data) &&
      event.data.find((item) => item?.proposal_id)?.proposal_id)
  );
}
...
// Handle new proposal submission (under review)
async function handleNewProposal(proposalId, eventDetails) {
  try {
    ...
    console.log(
      `📤 Sent creation notifications for proposal ${proposalId} to ${sentCount} chats`
    );
  } catch (error) {
    console.error(
...
async function handleProposalApproval(proposalId, eventDetails) {
  try {
    ...
    console.log(
      `📤 Sent approval notifications for proposal ${proposalId} to ${sentCount} chats`
    );
  } catch (error) {
    console.error(
...
async function fetchRecentProposals(count = 5) {
  try {
    ...
    const totalRes = await fetchWithTimeout(
      NEAR_RPC_URL,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    const proposalsRes = await fetchWithTimeout(
      NEAR_RPC_URL,
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
async function fetchRecentActiveProposals(count = 5) {
  try {
    ...
    if (!totalRes.ok) {
      throw new Error(
        `Failed to get active proposal count: ${totalRes.status}`
      );
    }
